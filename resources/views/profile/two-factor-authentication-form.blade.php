<x-jet-action-section>
    ...
    <x-slot name="description">
        {{ __('Agrega seguridad adicional a tu cuenta usando Autenticacion en dos pasos.') }}
    </x-slot>
    ...
    <x-slot name="content">
        <h3 class="text-lg font-medium text-gray-900">
            @if ($this->enabled)
                ...
                @else
                    {{ __('Tu tienes habilitada la autenticación en dos pasos.') }}
                @endif
            @else
                {{ __('Tu no tienes habilitada la autenticación en dos pasos.') }}
            @endif
        </h3>

        <div class="mt-3 max-w-xl text-sm text-gray-600">
            <p>
                {{ __('Cuando la autenticación en dos pasos está habilitada, se le pedirá un token seguro y aleatorio durante la autenticación. Puede recuperar este token de la aplicación Google Authenticator de su teléfono.') }}
            </p>
        </div>
        ...
        @if ($this->enabled)
            ...
            @if ($showingRecoveryCodes)
                <div class="mt-4 max-w-xl text-sm text-gray-600">
                    <p class="font-semibold">
                        {{ __('Almacene estos códigos de recuperación en un administrador de contraseñas seguro. Se pueden usar para recuperar el acceso a su cuenta si pierde su dispositivo de autenticación de dos factores.') }}
                    </p>
                </div>
